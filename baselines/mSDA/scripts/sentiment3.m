...
for j = 1:size(domains,1)
	...
	% test set is 1262 examples
	xe=[x(:, offset(5)+1:offset(5)+1262); allhx(:, offset(5)+1:offset(5)+1262)];
	xe=xe';
	ye=trg_y(1:1262);
	...
	xe=[x(:, offset(6)+1:offset(6)+8005); allhx(:, offset(6)+1:offset(6)+8005)];
	xe=xe';
	ye=trg_y2(1:8005);
	[pred,accuracy] = svmpredict(ye,xe,model);
	ev=metrics(ye,pred);
